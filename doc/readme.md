...
| [工程结构分析](develop/project_arc.md) | 开发者、维护者、代码审查人员 | 说明项目定位、顶层目录、源码模块、资源与国际化、插件体系、构建测试入口和主要风险点。 |
...
1. 先阅读 [工程结构分析](develop/project_arc.md)，确认模块边界、构建入口和测试入口。
2. UI、下载、插件、存储等改动可参考 [工程结构分析 - Codex 修改路由建议](develop/project_arc.md#7-codex-修改路由建议) 定位代码。
...
- 模块职责、目录结构、插件体系或构建入口变化时，同步更新 [工程结构分析](develop/project_arc.md)。
